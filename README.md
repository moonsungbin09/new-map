...
### Simple Frontend 재배포 확인 체크

- 홈(`/`)에서 소개 문구, 목록 영역, 등록 버튼이 보인다.
- 실패 문구 재현(비개발자용): 1) 인터넷을 잠깐 끄고 2) 홈 새로고침 → "목록을 불러오지 못했습니다."가 보이면 정상(확인 후 인터넷 다시 켜기).
- 목록 정상 기준: 항목이 보이거나, 항목이 0개여도 에러 없이 빈 목록으로 보인다.
- API 정상 기준: 브라우저에서 `/api/places`를 열었을 때 JSON이 보이고 오류 없이 200 응답이다.
- 등록 버튼 클릭 시 `/create.html`로 이동한다.
